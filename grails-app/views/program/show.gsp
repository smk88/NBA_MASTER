	...
	<body>
		...
		<div id="show-program" class="content scaffold-show" role="main">
			...
			<ol class="property-list program">
				...
				<g:if test="${programInstance?.department}">
				<li class="fieldcontain">
					<span id="department-label" class="property-label"><g:message code="program.department.label" default="Department" /></span>
					
						<span class="property-value" aria-labelledby="department-label"><g:link controller="department" action="show" id="${programInstance?.department?.id}">${programInstance?.department?.encodeAsHTML()}</g:link></span>
